...
async function fetchSuggestions(query: string, type: "all" | "movie" | "tv") {
  ...
  if (!response.ok) {
    ...
    throw new Error(payload?.error ?? "Suggestions could not be loaded.");
  }
...
function SuggestionList({
  ...
  mediaLabel: { movie: string; tv: string };
}) {
  if (!items.length) {
    return null;
  }
            ...
            >
              <div className="min-w-0">
                ...
                <div className="mt-1 flex items-center gap-2 text-xs text-muted-foreground">
                  {item.mediaType === "movie" ? <Film className="size-3.5" /> : <Tv2 className="size-3.5" />}
                  <span>{item.mediaType === "movie" ? mediaLabel.movie : mediaLabel.tv}</span>
...
export function AIComparePanel({ leftPreset }: { leftPreset?: { title: string; mediaType: "movie" | "tv" } }) {
  const { locale } = useLanguage();
  const text = locale === "en"
    ? {
        loadError: "The title comparison could not be loaded.",
        title: "Compare titles",
        description: "Compare tone, pace, complexity and audience fit between two titles. Suggestions appear while typing.",
        button: "Compare",
        all: "Movie or series",
        movie: "Movie",
        tv: "Series",
    ...
    : {
        loadError: "Der Titelvergleich konnte nicht geladen werden.",
      ...
      >
        ...
        {result ? (
          <div className="space-y-4">
            <div className="rounded-2xl border border-border/50 bg-background/50 p-4 text-sm text-muted-foreground">{result.data.shortVerdict}</div>
              ...
              <div className="overflow-x-auto">
                <div className="min-w-[40rem]">
                  <div className="grid grid-cols-[140px_minmax(0,1fr)_minmax(0,1fr)] bg-background/70 px-4 py-3 text-xs uppercase tracking-[0.16em] text-muted-foreground">
                    <div>{text.aspect}</div>
                    <Link href={result.resolved.left.href} className="truncate hover:text-primary">{result.resolved.left.title}</Link>
                    <Link href={result.resolved.right.href} className="truncate hover:text-primary">{result.resolved.right.title}</Link>
                  </div>
                  {result.data.comparison.map(row => (
                    <div key={row.label} className="grid grid-cols-[140px_minmax(0,1fr)_minmax(0,1fr)] gap-3 border-t border-border/40 px-4 py-3 text-sm">
